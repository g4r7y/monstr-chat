...
class TestDataStore implements DataStore {
  ...
  constructor() { this.#appData = null; this.#messages = null }
  async readAppData() { return Promise.resolve(this.#appData) }
  async writeAppData(appData: ChatAppData) { this.#appData = structuredClone(appData) }
  async readMessages() { return Promise.resolve(this.#messages) }
  async writeMessages(messages: ChatMessage[]) { this.#messages = structuredClone(messages) }
}
...
describe('chat controller', () => {
  ...
  describe('receive messages', () => {
    ...
    beforeAll(async () => {
      ...
      (receiveDms as Mock).mockImplementation( 
        (pubkey: string, privateKey: Uint8Array, pool: SimplePool, relays: string[], onMessage: (msg: ChatMessage)=>Promise<void>) => {
  ...
  describe('contacts', () => {
    ...
    test('set and get contacts', () => {
      ...
      const contact2 = { ...contact1, name: 'Jane', npub: 'npub1al0de7phghsqxyje4dn3lwya30akd255g0qt3xjdtreqhndtlm7quul2rj'};
      const contact3 = { ...contact1, name: 'Freddy', npub: 'npub1z7qa0cwpulrzg6yuj3cmm4pukxns5ak7fwdgmxlsgcmd6q0r537qcvq3nn'};
      ...
      expect(controller.getContactByNpub('npub1z7qa0cwpulrzg6yuj3cmm4pukxns5ak7fwdgmxlsgcmd6q0r537qcvq3nn')).toEqual(contact3);
  ...
  describe('user metadata', () => {
    ...
    test('user metadata event will update user profile', async () => {
      ...
      const baseEvent = { created_at: 1771370000, kind: 0, tags: [] };
      
      // set up mock for nip05 address query to simulate VALID nip05 address (profile query returns correct valid pubkey)
      (queryProfile as Mock).mockResolvedValue({ pubkey });
      // simulate user metadata event
      let profile = { name: 'Bugs bunny', about: 'Rabbit', nip05: 'bugs@lol.com' }
      const event1 = { 
        ...baseEvent, 
      ...
      profile = { name: 'Bugs bunny', about: 'Bad Rabbit', nip05: 'bad_address@lol.com' };
      // simulate user metadata event
      const event2 = { 
        ...baseEvent, 
        content: JSON.stringify(profile) 
      ...
      expect(controller.getSettings().profile).toEqual( { ...profile, nip05: null } );
    ...
    test('user metadata event will update contact profile', async () => {
      ...
      // set up mock for nip05 address query to simulate VALID nip05 address (profile query returns correct valid pubkey)
      (queryProfile as Mock).mockResolvedValue({ pubkey: friendPubkey});
      // simulate a user metadata event
      let profile = { name: 'My name is Steve', about: 'I like cheese', nip05: 'steve@lol.com' }
      const event1 = { 
        ...baseEvent,
        content: JSON.stringify(profile),
      }
      ...
      (queryProfile as Mock).mockResolvedValue( { pubkey: decode('npub1qnjcyscsu6sjf5q8cql2kwyyd4wlx09v62u2mf002xl04uand5aq7c8uf2').data });
      // simulate user metadata event
      profile = { name: 'I am Steve', about: 'I like cheese', nip05: 'bad_address@lol.com' }
      const event2 = { 
        ...baseEvent,
        content: JSON.stringify(profile),
      }
      ...
      expect(controller.getContactByName('Steve')!.profile).toEqual( { ...profile, nip05: null } );
    })
    ...
    test('lookup a user profile', async () => {
      ...
      const profile = { name: 'Alice', about: 'from Wonderland', nip05: 'alice@nostr.com'};
      const event = { 
      ...
      expect(result).toEqual( { ...profile, nip05: null });
    })
  })
  ...
  describe('relay metadata', () => {
    ...
    test('relay metadata events update user\'s local relay settings', async () => {
      ...
      const event1 = { 
        created_at: 1771370000, 
        kind: 1002, 
        tags: [ ['r', 'wss://relay1.com'],['r', 'wss://relay111.lol'] ],
        content: ''
      };
