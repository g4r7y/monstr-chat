...
export async function showPrompt(
  ...
  const response = await terminal.inputField({ minLength, maxLength: 512, cancelable: true, default: defaultResponse })
    .promise;
  terminal('\n');
...
export async function pressToContinue(question: string): Promise<void> {
  terminal(`${question}`);
  await terminal.singleLineMenu(['OK'], { selectedStyle: terminal.inverse }).promise;
}
...
export async function startScrollPane(lines: string[], headerHeight: number, footerHeight: number) {
  ...
  function renderBuffer() {
    let clipRect = new Rect({
      ...
      height: terminal.height - headerHeight - footerHeight
    });
    buffer.draw({
      dst: terminal,
      x: 0,
